...
export default async function AdminStoreDetailPage({ params }: { params: { storeId: string } }) {
  ...
  const [
    ...
    supabase.from('stores').select('*').eq('id', params.storeId).is('deleted_at', null).single(),
    supabase.from('profiles').select('name, email, role').eq('id', (await supabase.from('stores').select('owner_id').eq('id', params.storeId).single()).data?.owner_id ?? '').single(),
    supabase.from('ad_daily_reports').select('*').eq('store_id', params.storeId).gte('date', monthStart).lte('date', monthEnd),
    supabase.from('platform_connections').select('*').eq('store_id', params.storeId),
    supabase.from('ai_comments').select('*').eq('store_id', params.storeId).eq('approved', false).order('generated_at', { ascending: false }).limit(5),
    supabase.from('improvement_suggestions').select('*').eq('store_id', params.storeId).in('status', ['new', 'in_progress']).order('priority').limit(10),
    supabase.from('partner_notes').select('*, profiles(name)').eq('store_id', params.storeId).order('created_at', { ascending: false }).limit(10),
    supabase.from('activity_logs').select('*').eq('store_id', params.storeId).order('created_at', { ascending: false }).limit(10),
    supabase.from('lp_pages').select('id, title, status').eq('store_id', params.storeId).is('deleted_at', null),
    supabase.from('subscriptions').select('*').eq('store_id', params.storeId).single(),
    supabase.from('store_users').select('*, profiles(name, email, role)').eq('store_id', params.storeId),
  ])
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/admin/stores/${params.storeId}/lp`}>
            ...
            <CardContent className="space-y-2">
              ...
              <Link href={`/admin/stores/${params.storeId}/lp`} className="block pt-1">
